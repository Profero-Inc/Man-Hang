...
bool checkNcursesInstalledArch() {
    int result = system("pacman -Q ncurses > /dev/null 2>&1");
    return result == 0;
}
...
string RandomWord(){
    string tmpFile = "/tmp/tempword.txt";
    string cmd = "shuf -n 1 " + Difficulty + ".txt > " + tmpFile;
    int status = system(cmd.c_str());
    if (status != 0) {
        cout << "Failed to run shuf command." << endl;
        return "";
    }
    ...
    if (in.is_open()) {
        ...
        cout << "Failed to read from temp file." << endl;
    }
